...
class InntektMapperTest {
    ...
    public void allValuesSet() throws Exception {
        ...
        no.nav.foreldrepenger.oppslag.domain.Inntekt income = InntektMapper.map(inntekt);
        assertEquals(LocalDate.of(2017, 12, 13), income.getFrom());
        assertEquals(LocalDate.of(2017, 12, 14), income.getTo().get());
