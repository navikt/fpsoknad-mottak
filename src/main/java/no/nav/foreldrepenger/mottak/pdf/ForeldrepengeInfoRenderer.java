...
public class ForeldrepengeInfoRenderer {
    ...
    public float opptjening(Opptjening opptjening, List<Arbeidsforhold> arbeidsforhold, PDPageContentStream cos, float y) throws IOException {
        float startY = y;
    ...
    public float relasjonTilBarn(RelasjonTilBarnMedVedlegg relasjon, PDPageContentStream cos, float y) throws IOException {
        float startY = y;
        y -= renderer.addBlankLine();
    ...
    private String format(Arbeidsforhold arbeidsforhold) {
        return arbeidsforhold.getArbeidsgiverNavn() + ", fom " + textFormatter.date(arbeidsforhold.getFrom()) +
            " tom " + textFormatter.date(arbeidsforhold.getFrom()) + ", " + arbeidsforhold.getStillingsprosent() + "%";
    }
