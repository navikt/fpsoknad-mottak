    ...
    public Kvittering handle(ResponseEntity<FordelKvittering> leveranseRespons) {
        ...
        if (leveranseRespons.getStatusCode() == HttpStatus.ACCEPTED) {
            if (fpFordelKvittering instanceof PendingKvittering pending) {
                return handlePendingKvittering(leveranseRespons, timer, pending);
            }
            ...
            LOG.warn("Uventet responskode {} ved leveranse av søknad, gir opp (etter {}ms)",
                leveranseRespons.getStatusCode(), stop(timer));
            return new Kvittering();
        }
        return new Kvittering();
    }

    private Kvittering handlePendingKvittering(ResponseEntity<FordelKvittering> leveranseRespons,
                                               StopWatch timer,
        ...
        for (var i = 1; i <= fpfordelMax; i++) {
            LOG.info("Poller {} for {}. gang av {}, medgått tid er {}ms", pollURI, i, fpfordelMax,
                timer.getTime());
            ...
            LOG.info("Behandler poll respons {} etter {}ms", fpFordelResponse.getBody(), timer.getTime());
            switch (fpFordelResponse.getStatusCode()) {
                case OK -> {
                    ...
                    LOG.warn("Uventet kvittering {} for statuskode {}, gir opp (etter {}ms)",
                        fpFordelKvittering, fpFordelResponse.getStatusCode(), stop(timer));
                    return new Kvittering();
                }
                case SEE_OTHER -> {
                    FORDELT_KVITTERING.increment();
                    var fordelt = (FPSakFordeltKvittering) fpFordelKvittering;
                    return Kvittering.fordeltKvittering(fordelt);
                }
                default -> {
                    FEILET_KVITTERINGER.increment();
                    LOG.warn("Uventet responskode {} etter leveranse av søknad, gir opp (etter {}ms)",
                        fpFordelResponse.getStatusCode(), timer.getTime());
                    return new Kvittering();
                }
            }
        }
        LOG.info("Pollet FPFordel {} ganger, uten å få svar, gir opp (etter {}ms)", fpfordelMax, stop(timer));
