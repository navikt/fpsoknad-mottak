...
public class YtelseskontraktMapper {
	...
	public static Benefit map(Ytelseskontrakt kontrakt) {
		return new Benefit(kontrakt.getYtelsestype(), kontrakt.getStatus(),
		        CalendarConverter.toDate(kontrakt.getFomGyldighetsperiode()),
		        Optional.ofNullable(kontrakt.getTomGyldighetsperiode()).map(CalendarConverter::toDate));
	}
