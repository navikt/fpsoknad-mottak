...
public class FNRChildSelector implements ChildSelector {
	...
	public boolean isEligible(Fodselsnummer fnrMor,Barn barn) {
		return barn != null && barn.getFnr().getFnr().length() == 11
		        && FMT.parseDateTime(barn.getFnr().getFnr().substring(0, 6)).isAfter(DateTime.now().minusMonths(monthsBack));
	}
